...
const heroes = [
    ...
    {
        "name": "npc_dota_hero_dawn_breaker",
        "id": 135
    },
    {
...
async function fetchAvgStats(accountId) {
    ...
    try {
        const response = await axios.get(`https://api.opendota.com/api/players/${accountId}/recentMatches?api_key=8b0e0b9d-ef94-4752-95c9-3aecfb0e95c3`);
        ...
        const totalDuration = validMatches.reduce((sum, match) => sum + match.duration, 0);
...
async function fetchTopHeroes(accountId) {
    ...
    try {
        const response = await axios.get(`https://api.opendota.com/api/players/${accountId}/heroes?api_key=8b0e0b9d-ef94-4752-95c9-3aecfb0e95c3`);
        ...
        const topHeroes = heroes
            ...
            .map(hero => {
                ...
                const winRate = hero.games > 0 ? ((hero.win / hero.games) * 100).toFixed(2) : '0.00';
        ...
        console.error(`Error fetching hero data for account ID ${accountId}:`, error);
        const emptyHeroData = {};
        playerHeroDataCache.set(accountId, emptyHeroData);
        notFoundCache.set(accountId, true); // Cache this account ID as "not found"
        return emptyHeroData;
...
gsiServer.events.on('newclient', (client) => {
    ...
    client.on('newdata', async (data) => {
        ...
        if (data.draft) {
            ...
            gameData.draft.activeTeamRemainingTime = draftData.activeteam_time_remaining;
            ...
            if (draftData.team2) {
                for (let i = 0; i < 5; i++) { // Iterate over picks
                    const pickId = draftData.team2[`pick${i}_id`];
                    if (pickId && pickId !== 0) {
                        gameData.draft.radiant.picks[`pick${i + 1}`] = getHeroNameById(pickId);
                    }
                }

                for (let i = 0; i < 7; i++) { // Iterate over bans
        ...
        for (const [index, team] of teams.entries()) {
            ...
            if (data.player && data.player[team]) {
                for (const playerKey of Object.keys(data.player[team])) {
                    ...
                    const topHeroes = player.accountid ? await fetchTopHeroes(player.accountid) : {};
