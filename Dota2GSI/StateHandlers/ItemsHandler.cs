...
namespace Dota2GSI
{
    public class ItemsHandler : EventHandler<DotaGameEvent>
    {
        ...
        private void OnItemsUpdated(DotaGameEvent e)
        {
            ...
            {
                dispatcher.Broadcast(new ItemDetailsChanged(evt.New.LocalPlayer, evt.Previous.LocalPlayer, local_player_details));
            }
            ...
            foreach (var team_kvp in evt.New.Teams)
            {
                foreach (var player_kvp in team_kvp.Value)
                {
                    ...
                    {
                        dispatcher.Broadcast(new ItemDetailsChanged(player_kvp.Value, previous_hero_details, player_details));
                    }
                }
            }
        }
