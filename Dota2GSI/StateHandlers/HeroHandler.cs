...
namespace Dota2GSI
{
    public class HeroHandler : EventHandler<DotaGameEvent>
    {
        ...
        private void OnHeroUpdated(DotaGameEvent e)
        {
            ...
            {
                dispatcher.Broadcast(new HeroDetailsChanged(evt.New.LocalPlayer, evt.Previous.LocalPlayer, local_player_details));
            }
            ...
            foreach (var team_kvp in evt.New.Teams)
            {
                foreach (var player_kvp in team_kvp.Value)
                {
                    ...
                    {
                        dispatcher.Broadcast(new HeroDetailsChanged(player_kvp.Value, previous_hero_details, player_details));
                    }
                }
            }
        }
